...
from tkinter import LabelFrame, IntVar, StringVar, Canvas
...
class Picker(ttk.Frame):
    ...
    def __init__(self, master=None, activebackground='#b1dcfb', values=[], entry_wid=None, activeforeground='black',
        ...
        F = LabelFrame(self)
        F.pack(fill='x')
        self.canvas = Canvas(F, scrollregion=(0, 0, 500, (len(self._values) * 24)))
        vbar = ttk.Scrollbar(F, orient='vertical')
...
class Combopicker(ttk.Entry, Picker):
    def __init__(self, master, values=[], entryvar=None, entrywidth=None, entrystyle=None, onselect=None,
                 ...
                 selectforeground='white', borderwidth=1, relief="solid", state='normal'):
        ...
        ttk.Entry.__init__(self, master, textvariable=self.entry_var, **entry_config, state=state)
    ...
    def current_value(self):
        try:
            value = self.entry_var.get().split(",")
            return value
        except ValueError:
            return None
